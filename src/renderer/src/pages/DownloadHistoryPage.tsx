...
export default function DownloadHistoryPage() {
  ...
  const downloading = tasks.filter((t) => t.status === 'pending' || t.status === 'downloading')
      ...
      {completed.length > 0 && (
        <section className="mb-5">
          ...
          <div className="bg-apple-card rounded-xl border border-apple-border-subtle divide-y divide-apple-border-subtle">
            {completed.map((task) => (
              <div key={task.id} className="flex flex-wrap items-center gap-3 px-4 py-3 sm:flex-nowrap">
                ...
                <button
                  onClick={() => {
                    if (api.target === 'web') api.downloadArtifact(task.id)
                    else void api.openFolder(task.type === 'images' ? 'pics' : 'novels')
                  }}
                  className="text-apple-tertiary hover:text-apple-accent transition-colors px-1"
                  title={api.target === 'web' ? '下载文件' : '打开所在文件夹'}
                >
                  {api.target === 'web' ? (
