...
- 外部服务密钥、Token、用户密码、Cookie、Authorization、签名和真实加密载荷不得进入代码、日志、fixture、快照、文档或 Git 历史。用于本地固定算法的公开派生材料不视为秘密，但必须明确说明其仅用于避免明文展示，不构成安全边界。
...
- 普通设置存放在 `settings.toml`；账号、密码和 Cookie 使用项目内置的固定本地算法加密后写入 `secrets.enc`，不得写入明文凭证文件。该方案只防止直接查看文件，不依赖系统钥匙串，也不作为抵御本机攻击者的安全边界。
...
- `docs/UI_DESIGN_GUIDE.md` 是 renderer 长期 UI 设计依据。新增或修改可见界面前先读取相关章节，并保持“安静的书库工具”方向。
- 保持稳定应用壳和内容优先层级；每页只设置一个最高强调等级的主动作，避免卡片套卡片、重复装饰和营销页式视觉。
- 可见布局同时兼容 `1200 × 800` 默认窗口和 `900 × 600` 最小窗口，不得出现页面级横向滚动、主要操作不可见或控件重叠。
- 用户可见的异步操作应覆盖 loading、error 和 empty 状态；组件卸载时清理 IPC 监听器。
- 主要流程保持键盘可用、焦点可见；图标按钮提供可访问名称，颜色不作为状态的唯一表达。
...
- 可见 UI 修改的 smoke test 按影响范围检查 `1200 × 800` 和 `900 × 600`，并覆盖受影响的默认、loading、empty、error、disabled 和长内容状态。
- 只有发布任务才运行 `dist:*`。

## 文档维护

- 使用方式、公开接口或长期架构约束变化时，更新对应持久文档。
- renderer 的长期视觉、布局、组件或交互规则变化时同步更新 `docs/UI_DESIGN_GUIDE.md`；临时参考、竞品链接和一次性探索稿不得写入该指南。
